...
class SpecularCubemapFilter(
    ...
    private class ChunkIterable(maxNumberOfColorAttachments: Int) :
        Iterable<Chunk?> {
    ...
    fun update(images: Array<Image>) {
        try {
            ...
            for (i in 0 until NUMBER_OF_CUBE_FACES) {
                ...
                if (image.format != ImageFormat.RGBA_FP16) {
                    throw IllegalArgumentException(
                        "Unexpected image format for cubemap: " + image.format
                    )
                }
    ...
    private fun createShaders(
        render: ARRenderer,
        chunks: SpecularCubemapFilter.ChunkIterable
    ): Array<Shader?> {
        ...
        for (chunk: Chunk? in chunks) {
            ...
            for (location in 0 until chunk!!.chunkSize) {
                defines[ATTACHMENT_LOCATION_DEFINES.get(chunk.firstFaceIndex + location)] =
        ...
        for (shader: Shader? in shaders) {
            for (i in importanceSampleCaches.indices) {
                val cache: Array<ImportanceSampleCacheEntry?>? = importanceSampleCaches[i]
    ...
    private fun generateImportanceSampleCaches(): Array<Array<ImportanceSampleCacheEntry?>?> {
        val result: Array<Array<ImportanceSampleCacheEntry?>?> = arrayOfNulls<Array<ImportanceSampleCacheEntry?>>(numberOfMipmapLevels - 1)
        for (i in 0 until (numberOfMipmapLevels - 1)) {
            ...
            val cache: ArrayList<ImportanceSampleCacheEntry> =
                ArrayList<ImportanceSampleCacheEntry>(
                    numberOfImportanceSamples
                )
            var weight = 0f
            ...
            result[i] =
                arrayOfNulls<ImportanceSampleCacheEntry>(
                    cache.size
                )
    ...
    init {
        ...
        try {
            ...
            mesh = Mesh(
                ...
                null, arrayOf<VertexBuffer?>(coordsBuffer)
            )
        } catch (t: Throwable) {
            close()
            throw t
    ...
    companion object {
        ...
        private val COMPONENTS_PER_VERTEX = 2
        private val NUMBER_OF_VERTICES = 4
        private val FLOAT_SIZE = 4
        private val COORDS_BUFFER_SIZE = COMPONENTS_PER_VERTEX * NUMBER_OF_VERTICES * FLOAT_SIZE
        private val NUMBER_OF_CUBE_FACES = 6
